...
カレンダーがログインを求める場合は、Googleカレンダー側で対象カレンダーの公開設定が有効になっていない可能性があります。coasobiのGoogleアカウントでカレンダーの「設定と共有」を開き、「予定のアクセス権限」で一般公開の設定を確認してから、埋め込み用URLを使ってください。公開カレンダーには、予約者名、子どもの情報、住所詳細などの個人情報を入れず、「空きあり」「予約済み」程度の表記にしてください。
...
- イベント: 1組100円程度（場所代実費）
- 支払い方法: 利用者さまの希望に合わせて、現金、イチコ、振り込み（オンラインカウンセリングと別日の事前面談は現金を除く）
